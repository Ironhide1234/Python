...
class RegularTicket:
    ...
    def generateTicket(person, event, eventDate, basePrice, student=False):
        if not(isinstance(eventDate, str) and  isinstance(event, str) and  isinstance(basePrice, int) and  isinstance(person, str) and  isinstance(student, bool)):
...
class AdvanceTicket(RegularTicket):
    ...
    @price.setter
    def price(self, price):
        self.__price = price * COEF_ADVANCE
...
class StudentTicket(RegularTicket):
    ...
    @price.setter
    def price(self, price):
        self.__price = price * COEF_STUDENT


print(RegularTicket.createTicket("data.json"))
